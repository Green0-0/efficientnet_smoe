...
class EfficientNetB0_DeepMoE(nn.Module):
    def __init__(
        ...
        for stage_idx, (
            ...
            stride,
            kernel_size,
        ) in enumerate(self.b0_config):
            for i in range(repeats):
                s = stride if i == 0 else 1
                drop_prob = (
                    stochastic_depth_prob * float(block_idx) / max(1, total_blocks - 1)
                )
    ...
    def forward(self, x):
        ...
        x = self.head(x)
        active_pct = (
            (active_experts / max(1.0, total_experts))
            if total_experts > 0
...
# ==========================================
# 2. DATA LOADING (iNat 2019, seeded 50/50 split)
# ==========================================
def get_dataloaders(data_dir, batch_size, num_workers=8):
    ...
    all_indices = torch.randperm(len(full_train_dataset), generator=g).tolist()

    # 50% train-pool / 50% held-out test (never used)
    train_pool_size = len(all_indices) // 2
    train_pool_indices = all_indices[:train_pool_size]

    # Of train-pool: 90% train / 10% val
    val_size = int(0.1 * len(train_pool_indices))
    val_indices = train_pool_indices[:val_size]
    train_indices = train_pool_indices[val_size:]
...
def objective(trial):
    ...
    lambda_g = trial.suggest_float("lambda_g", 1e-6, 0.001, log=True)
    ...
    run = wandb.init(
        ...
        config={
            ...
            "lambda_g": lambda_g,
            "mu": mu,
            "model": "EfficientNet-B0 + DeepMoE (iNat 2019, 1010 classes)",
    ...
    model = EfficientNetB0_DeepMoE(
        num_classes=num_classes, moe_start_stage=moe_start_stage
    ).to(device)
        ...
        for epoch in range(TARGET_EPOCHS):
            ...
            for i, (images, labels) in enumerate(train_loader):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                    ...
                    loss = (
                        loss_main + mu * loss_aux + lambda_g * l1_loss
                    ) / grad_accum_steps
...
if __name__ == "__main__":
    ...
    pruner = optuna.pruners.MedianPruner(
        n_startup_trials=20, n_warmup_steps=10, interval_steps=5
    )
